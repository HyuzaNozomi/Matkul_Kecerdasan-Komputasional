...
def train_taxi_agent(total_episodes):
    ...
    for episode in range(total_episodes):
        ...
        while not done:
            #METODE Strategi Epsilon-Greedy
            #Menghasilkan angka acak antara 0.0 hingga 1.0 sebagai ambang batas keputusan
            exploration_rate_threshold = random.uniform(0, 1)
            
            if exploration_rate_threshold > epsilon:
                #Agen memilih aksi yang memiliki nilai probabilitas (skor Q) paling tinggi 
                action = np.argmax(q_table[state, :])
            else:
                # Agen mengabaikan Q-Table dan memilih aksi secara acak dari environment (antara 0 sampai 5 untuk Taxi-v4).
        ...
        #Rumus peluruhan eksponensial untuk mengecilkan nilai epsilon perlahan-lahan.
        #Semakin besar nilai 'episode', nilai epsilon akan semakin mendekati 'min_epsilon'.
